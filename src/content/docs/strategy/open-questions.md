...
| Q10 | Enterprise sizing — *resolved:* ~50% of teams are 2-7, 84% of API teams are 1-9 → **free cap = up to 5**. Residual: surveys measure *immediate-team*, not org-*workspace-seat* counts | Pin the exact 3-vs-4-vs-5 optimum from real workspace seat data when available | Low (5 is a fine launch value) |
| Q11 | Disruptive features — *resolved:* validated = encrypted-shareable secrets, **API catalog/discoverability** (34% can't find APIs), config-as-code, spec-drift (parity); weak = response diffing; **not validated** = contract testing, governance-linting, AI-authoring | A final confirm-or-kill pass for the 3 not-validated; otherwise don't build them | Low |
| Q12 | **Request diagnostics adoption** — the *demand* for DNS/TCP/TLS/timing diagnosis is validated, but will developers actually use it in-app (vs `curl`/`dig`), and is there any *separate* willingness-to-pay? | Ship the Tier-1 request-centric flow ([Feature 19](/rambo/features/19-request-diagnostics/)); measure repeated use before funding Tier-2 ping/traceroute/MTR or any remote-probe add-on | Low — enhancer, not a wedge |
